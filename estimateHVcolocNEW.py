...
stime = UTCDateTime('2017-190T00:00:00.0')
etime = UTCDateTime('2017-193T00:00:00.0')
...
def proceve(eve, sta, debug=False):
    ...
    fstring = 'Results/JUNK' + net + '_' + sta + '_' + str(eve.origins[0].time.year) + '_' + str(eve.origins[0].time.julday).zfill(3) + \
            ...
            '_Results.csv'
    feve = open(fstring,'w')
    feve.write('sta, loc, year, day, f0, distance, azimuth, corr, mag, mHV, stdHV, pRV, pNV, pEV \n')
    if debug:
    ...
    for loc in locs:
        ...
        plt.suptitle(st[0].stats.network + ' ' + st[0].stats.station + ' Distance: ' + str(int(round(disdeg,0))) + ' degrees')
        for idx, f0 in enumerate(f0s):
            print('Here is f0:' + str(f0))
            ...
            corr = np.interp(range(len(HilbertV)),oldx, corr)
            env = envelope(st2.select(component="R")[0].data)*envelope(HilbertV)
            HV = np.log10(envelope(st2.select(component="R")[0].data)/envelope(HilbertV))
            env *= 1./np.max(np.abs(env))
            t = np.asarray(range(len(HilbertV)))
            ...
            mHV = np.mean(HV2)
            print('Here is mHV for ' + str(f0) + ' ' + str(mHV) + ' ' + loc)
            
            stdHV = np.std(HV2)
            print('Here is stdHV for ' + str(f0) + ' ' + str(stdHV) + ' ' + loc)
            ...
            try:
                ...
                if idx ==0:
                    ax1 = plt.subplot(221)
                    plt.title('100 s Period')
                    plt.ylabel('Velocity (nm/s)')
                    plt.ylim((-2000., 2000.))
                    plt.yticks([-1500,-1000, -500, 0, 500, 1000, 1500])
                    plt.tick_params(labelbottom=False)   
                    plt.text(10., 1700., '(a)')
                else:
                    plt.subplot(222)
                    plt.title('50 s Period')
                    plt.ylim((-2000., 2000.))
                    plt.yticks([-1500, -1000, -500, 0, 500, 1000, 1500])
                    plt.tick_params(labelbottom=False)  
                    plt.text(10., 1700., '(b)')
                    #plt.tick_params(labelleft=False)   
                plt.plot(t, HilbertV*10**9,linewidth=2.5, label=' Shifted Vertical: ' + loc, linestyle=lines)
                plt.xlim((min(t),max(t)))
                plt.plot(t,st2.select(component="R")[0].data*10**9, linewidth=2.5, label='Radial: ' + loc, linestyle=lines)
                ...
                if loc =='00':
                    ...
                    col='C9'
                plt.plot(t, np.log10(HV), label=loc + ' H/V' , linewidth=2.5, color=col, linestyle=lines)
    ...
    labels += labels2
    ax = plt.gca()
    fig.legend(handles, labels, loc = 'lower center', ncol = 3, fontsize = 20, labelspacing=1)
