...
for gidx, sta in enumerate(stas):
    ...
    if True:
        ...
        for idx, per in enumerate([150, 100, 75, 50, 25]):
            ...
            plt.plot(azis[(locs == '00') & (p0s == per) ]*180./np.pi, HVs[(locs == '00') & (p0s == per)],'.', markersize=12, alpha=.1, color='blue', label = '00 Measurement')
            plt.plot(azis[(locs == '10') & (p0s == per)]*180./np.pi, HVs[(locs == '10') & (p0s == per)],'.', markersize=12, alpha=.1, color='orange', label = '10 Measurement')
            if idx == 2:
            ...
            plt.errorbar(angs*180./np.pi, m00,  yerr=std00,marker ='.', markersize=18, color='blue', label='00 Mean',linestyle='')
            plt.errorbar(angs*180./np.pi + 2., m10, yerr=std10, marker = '.', markersize=18, color='orange', label= '10 Mean',linestyle='')
        ...
        plt.subplot(515)
        plt.legend(ncol=4, loc='center', bbox_to_anchor=(0.5, -0.44))
        plt.xlabel('Back-Azimuth (degrees)')
        #plt.tight_layout()
        plt.savefig( sta + '_Azimuth.jpg',format='JPEG', dpi=400)
        #plt.show()
        plt.clf()
        plt.close()
