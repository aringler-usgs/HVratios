...
def computeA(sta, freq, loc, quad, debug = True):
    files = glob.glob('NEWRESULTS2/*' + sta + '*Results.csv')
    azis = []
    amps = []
    hvs = []
    EV =[]
        ...
        with open(curfile,'r') as f:
            for line in f:
                if (loc in line.split(',')[1]) and (freq == float(line.split(',')[4])):
                    ...
                    NV.append(float(data[-4]))
                    EV.append(float(data[-2]))
                    hvs.append(float(data[-6]))
    #print(hvs)
    HV = np.mean(hvs) 
    ...
    for idx, pair in enumerate(zip(azis, NV, EV, hvs)):
        ...
        if pair[0] >= quad[0] and pair[0] <= quad[1]:
            ampN = (pair[1]/pair[3])/(-((2.*np.pi*freq*4000.)/9.81))
            ampE = (pair[2]/pair[3])/(-((2.*np.pi*freq*4000.)/9.81))
    ...
    sol1 = lsq_linear(A1,x1, bounds=(-1.,1.))
    sol1 = sol1.x
    print(sol1)
    sol2 = lsq_linear(A2,x2, bounds=(-1.,1.))
    sol2 = sol2.x
    print(sol1)
    ...
    resi1 = np.matmul(A1,sol1) - x1
    neve = len(resi1)
    resi1 = np.std(resi1)
    resi1 = 100*(1-np.var(np.matmul(A1,sol1)-x1)/np.var(x1))
    pinv2 = np.linalg.pinv(A2)
    corrs2 = np.matmul(pinv2,x2)
    resi2 = 100*(1-(np.var(np.matmul(A2,sol2)-x2)/np.var(x2)))
    
    del A1
    del A2
    return corrs1, corrs2, neve,resi1, resi2
    
stas = glob.glob('NEWRESULTS2/*.csv')
    ...
    for loc in ['00','10']:
        for freq in [0.00666666666667, 0.01, 0.0133333333333, 0.02, 0.04]:
            try:
                ...
                f2.write(sta + ', ' + loc + ', ' + str(freq) + ', Quad 1, ' + str(ln) + ', A1, ' + str(A1[0]) +  ', ' + str(A1[1]) + ', ' + str(A1[2]) + ', ' + str(resi1) + ' \n')
                f2.write(sta + ', ' + loc + ', ' + str(freq) + ', Quad 1, ' + str(ln) + ', A2, ' + str(A2[0]) +  ', ' + str(A2[1]) + ', ' + str(A2[2]) + ', ' + str(resi2) + ' \n')
