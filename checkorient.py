...
def proceve(eve, sta, debug=False):
    ...
    if disdeg <=50. or disdeg >=120.:
        return
    fstring = 'AZIS2/' + net + '_' + sta + '_' + str(eve.origins[0].time.year) + '_' + str(eve.origins[0].time.julday).zfill(3) + \
            '_' + str(eve.origins[0].time.hour).zfill(2) + '_' + str(eve.origins[0].time.minute).zfill(2) + \
            '_Results.csv'
    feve = open(fstring,'w')
    feve.write('sta, loc, year, day, f0, distance, azimuth, corr, mag, theta, corr, AR, AV, AT\n')
    if debug:
    ...
    for f0 in f0s:
        for loc in locs:
            ...
            st2 = finalfilter(st,f0,bazi,astime,aetime,True)
            good[loc] = st2.copy()
        # Now we have the terms we can try and check what we have
        #print(good)
        for loc in locs:
            print('Here is loc' + loc)
            if loc == '00':
                continue
            else:
                def checkterms():
                    ...
                    corrs =[]
                    for angle in angles:
                        temp = good[loc].select(component="R")[0].data*np.cos(np.radians(angle))
                        temp += -good[loc].select(component="T")[0].data*np.sin(np.radians(angle))
                        lag, corr = xcorr(ref,temp, 0, full_xcorr=False)
                        corrs.append(corr)
                    angle = angles[np.argmax(corrs)]
                    amps = {}
                    for comp in ['Z','R','T']:
                        amps[comp] = np.sum((good[loc].select(component=comp)[0].data)**2)
                        amps[comp] *= 1./np.sum((good['00'].select(component=comp)[0].data)**2)
                    corr = max(corrs)
                    return angle, corr, amps
                try:
                    ang, corr, amps = checkterms()
                except:
                    continue
                #print('Here is the angle: ' + str(ang) + ' ' + str(corr))
            #feve.write('sta, loc, year, day, f0, distance, azimuth, corr, mag, theta, corr, AR, AV, AT\n')
                print(amps['Z'])
                print(amps['R'])
                print(amps['T'])
                feve.write(sta + ', ' + loc + ', ' + str(eve.origins[0].time.year) +', ' + 
                            ...
                            + ', ' + str(ang) + ', ' + str(corr) + ', ' + str(amps['R']) + ', '+ str(amps['Z']) + ', ' + str(amps['T']) + '\n')
